...
export function ResultsPanel() {
  ...
  const filteredRows = useMemo(() => {
    return flatRows.filter((r) => {
      ...
      if (dateFilter && localDateKey(r.start_time) !== dateFilter) return false;
      return true;
    });
  }, [flatRows, userFilter, matchFilter, dateFilter]);

  if (loading) {
    return <p className="text-sm text-slate-400">Cargando resultados…</p>;
  }
        ...
        <table className="w-full min-w-[980px] text-left text-xs">
          ...
          <tbody>
            {filteredRows.map((r, idx) => {
              const icon =
                r.result_indicator === "correct" ? "✅" : r.result_indicator === "incorrect" ? "❌" : "⚪";
