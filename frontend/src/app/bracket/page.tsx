...
export default function BracketPage() {
  ...
  if (!data) {
    return <div className="min-h-screen bg-pitch px-4 py-8 text-slate-300">Cargando cuadro…</div>;
  }
  ...
  return (
    <div className="mx-auto min-h-screen max-w-5xl bg-pitch px-4 py-8 text-white">
      ...
      <header className="mb-6">
        <h1 className="text-2xl font-bold">Cuadro eliminatorio</h1>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {round.matches.map((row) => {
                  ...
                  return (
                    <div
                      key={m.id}
                      className="rounded-xl border border-slate-700 bg-card p-4"
                    >
                      {m.round ? (
                      ...
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex min-w-0 items-center gap-2">
                          {!isTbdCode(m.team_home_code) ? (
                            <img src={flagUrl(m.team_home_code)} alt="" className="h-5 w-7 rounded-sm object-cover" />
                          ) : (
                            <span className="flex h-5 w-7 items-center justify-center rounded bg-slate-700 text-[10px]">?</span>
                          )}
                        ...
                        <div className="flex min-w-0 items-center justify-end gap-2">
                          ...
                          {!isTbdCode(m.team_away_code) ? (
                            <img src={flagUrl(m.team_away_code)} alt="" className="h-5 w-7 rounded-sm object-cover" />
                          ) : (
                            <span className="flex h-5 w-7 items-center justify-center rounded bg-slate-700 text-[10px]">?</span>
                          )}
                        </div>
                      ...
                      {row.bet_count > 0 && popLabel ? (
                        <p className="mt-2 rounded-lg bg-slate-800/80 px-2 py-1.5 text-xs text-slate-200">
                          Grupo: <span className="font-semibold text-primary">{popLabel}</span> ({row.popular_pct}% ·{" "}
                          {row.bet_count} apuestas)
                        </p>
                      ) : (
